...
for seed in [1, 2, 3, 4]:        
    if seed == 1:
        experiment_version = 'exp_11'
    else:
        experiment_version = 'exp_11' + '_seed_' +str(seed) # ph 30
    pred_window = 6

    # experiment_version = 'exp_11_ph_60'
    # pred_window = 12
    CONF = {
    ...
    population_model = LR(CONF, is_load_model=True)
    ...
    for r, load_dataset in enumerate(dataset_list):
        ...
        population_model = test_log.load_scikit_model(population_model, load_dataset, name)
        ...
        for c, test_dataset in enumerate(dataset_list):
            ...
            for pid in data_proc.dataset2test_pid2data_npy[test_dataset].keys():
                output_dict = update.cross_test_inference_for_scikit(population_model, pid, test_dataset)
