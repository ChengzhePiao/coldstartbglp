...
for seed in [1, 2, 3, 4]:    
    if seed == 1:
        experiment_version = 'exp_4'
    else:
        experiment_version = 'exp_4' + '_seed_' +str(seed) # ph 30
    pred_window = 6

    # experiment_version = 'exp_4_ph_60'
    # pred_window = 12
    CONF = {
        'meta': 'MetaSGD',
        'fast_lr':1e-1,
    ...
    if CONF['meta'] == 'MAML':
        population_model = MAML(population_model, lr=CONF['fast_lr'], first_order = False)
    elif CONF['meta'] == 'MetaSGD':
        population_model = MetaSGD(population_model, lr=CONF['fast_lr'], first_order = False)
    else:
        print('error in meta')
